...
var _sendResponse = __webpack_require__(1);

var _emptyBucket = __webpack_require__(46);
...
const handler = exports.handler = async event => {
  ...
  if (event.RequestType === 'Create' || event.RequestType === 'Update') {// Nothing to do here - create and update should always succeed
  } else if (status === 'SUCCESS') {
    try {
      await (0, _emptyBucket.emptyBucket)(bucketName);
    } catch (err) {
...
exports.sendResponse = undefined;
 ...
 */
const sendResponse = exports.sendResponse = async props => {
  const body = {
    Status: props.status,
    Reason: props.reason,
  ...
  await _axios2.default.put(props.responseUrl, responseBody, {
    data: responseBody,
    headers: {
      'content-type': '',
...
const emptyBucket = exports.emptyBucket = async bucketName => {
